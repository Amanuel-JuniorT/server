...
class AuthManager extends Controller
{
    ...
    public function getUserProfile(Request $request)
    {
        try {
            ...
            if ($user->role === 'driver') {
                $user->load('driver.vehicle', 'driver.location');
            } else {
                $user->load('driver');

                // Add passenger stats
                $tripsCount = \App\Models\Ride::where('passenger_id', $user->id)
                    ->where('status', 'completed')
                    ->count();

                $averageRating = \App\Models\Rating::where('to_user_id', $user->id)
                    ->avg('score') ?? 5.0;

                $user->trips_count = $tripsCount;
                $user->user_rating = round($averageRating, 1);
                $user->user_points = 0; // Placeholder

                // Live streak/referral data for Rewards screen
                $streakEnabledVal = \App\Models\SystemConfiguration::where('key', 'streak_enabled')->value('value');
                $streaksEnabled = $streakEnabledVal && !in_array(strtolower($streakEnabledVal), ['false', '0', 'off', 'no']);
