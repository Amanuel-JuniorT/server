...
class DispatchRideJob implements ShouldQueue
{
    ...
    public function handle(UnifiedNotificationService $notificationService): void
    {
        ...
        if (!empty($results)) {
            ...
            Log::info("DispatchRideJob VALID DRIVERS IDs: " . json_encode($validDriverUserIds));
            ...
            if(!empty($validDriverUserIds)){
                foreach($validDriverUserIds as $driverUserId){
                    ...
                    if($this->ride->status === 'accepted'){
                        ...
                        $this->ride->update([
                            'rejected_driver_ids' => array_push($this->ride->rejected_driver_ids, $driverUserId)
                        ]);
                        continue;
                    }
                }
